...
import os
os.makedirs('./adj_graphs', exist_ok=True)
...
def _get_llm_components(model_path='./Llama3-8B'):
    global _LLM_TOKENIZER, _LLM_MODEL
    if _LLM_TOKENIZER is None or _LLM_MODEL is None:
        if not os.path.exists(model_path):
            raise FileNotFoundError(
                f"LLM edge-decision model path not found: {model_path}. "
                "Use the standard dataloader_detection.py path unless this experimental LLM loader is intentional."
            )
...
FILEMARKER_DIR = REPO_ROOT.joinpath('data/file_markers_detection')
...
def summarize_eeg_features(eeg_data):
    features = {
        ...
        "q25": np.percentile(eeg_data, 25),
        "q75": np.percentile(eeg_data, 75),
        "skew": skew(eeg_data),
        "kurtosis": kurtosis(eeg_data),
        "range": np.ptp(eeg_data),
        "absmean": np.mean(np.abs(eeg_data)),
        "energy": np.sum(eeg_data ** 2),
        "zerocross": np.sum(np.diff(np.sign(eeg_data)) != 0),
        "argmin": int(np.argmin(eeg_data)),
        "argmax": int(np.argmax(eeg_data)),
    }
    N = len(eeg_data)
    for name, slc in zip(["first", "middle", "last"], [slice(0, N//3), slice(N//3, 2*N//3), slice(2*N//3, None)]):
        part = eeg_data[slc]
        features[f"{name}_mean"] = np.mean(part)
        features[f"{name}_std"] = np.std(part)
        features[f"{name}_max"] = np.max(part)
        features[f"{name}_min"] = np.min(part)
    return ", ".join([f"{k}={v:.3f}" for k, v in features.items()])

def llm_edge_decision(eeg_data_i, eeg_data_j):
    feats_i = summarize_eeg_features(eeg_data_i)
    feats_j = summarize_eeg_features(eeg_data_j)
    prompt = (
        f"Given two EEG channel time series, each summarized by many statistics. "
        f"Decide if there is a strong physiological connection (YES or NO only):\n"
        f"Channel 1: {feats_i}\n"
        f"Channel 2: {feats_j}\n"
    ...
    answer = decoded.split("Answer:")[-1].strip().upper()
...
################## New Added ###############
        ...
        MAX_TEST_SAMPLE = 1
        if self.split == 'train':
            self.file_tuples = self.file_tuples[:MAX_TRAIN_SAMPLE]
            self.size = len(self.file_tuples)
        elif self.split == 'dev':
            self.file_tuples = self.file_tuples[:MAX_DEV_SAMPLE]
            self.size = len(self.file_tuples)
        elif self.split == 'test':
            self.file_tuples = self.file_tuples[:MAX_TEST_SAMPLE]
            self.size = len(self.file_tuples)
###################End new adds#####################
    ...
    def __getitem__(self, idx):
        ...
        if self.graph_type == 'individual':
            ...
            if os.path.exists(cache_file_path):
                ...
                for time_step in range(eeg_clip.shape[0]):
                    data_t = eeg_clip[time_step]  # (num_ch, points)
                    num_ch = data_t.shape[0]
                    adj_mat = np.zeros((num_ch, num_ch), dtype=np.float32)
                    for i in range(num_ch):
                        for j in range(num_ch):
                            if i == j:
                                ...
                                # 1. 先Llama3-8B判断是否有边
                                has_edge = llm_edge_decision(data_t[i], data_t[j])
                                if has_edge:
                                    # 2. Llama判有边时再计算权重（如Pearson）
                                    try:
                                        pearson = pearsonr(data_t[i], data_t[j])[0]
                                        adj_mat[i, j] = np.abs(pearson)   # 你可自定义权重规则
                                    except Exception:
                                        adj_mat[i, j] = 0.0  # 保险处理
                                else:
                                    adj_mat[i, j] = 0.0
                    # 下游不变
        ...
        np.save(f'./adj_graphs/clip_{idx}.npy', adj_mat_seq.numpy() if hasattr(adj_mat_seq, 'numpy') else adj_mat_seq)
