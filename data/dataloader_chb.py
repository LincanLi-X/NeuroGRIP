...
FILEMARKER_DIR = REPO_ROOT.joinpath('data/file_markers_chb_mit')
FREQUENCY = 256
...
def parseTxtFiles(split_type, task, clip_len, cv_seed=123, marker_dir=None):
    marker_dir = Path(marker_dir) if marker_dir is not None else FILEMARKER_DIR
    label_file_path = marker_dir.joinpath(
        split_type + '_' + task + '_' + str(clip_len) + 's.txt')
    if not label_file_path.exists():
        raise FileNotFoundError(
            f"CHB-MIT marker file not found: {label_file_path}. "
            "Create marker files or pass --chb_marker_dir to main.py."
        )
...
class SeizureDataset(Dataset):
    def __init__(
        ...
        self.file_tuples = parseTxtFiles(split, task, max_seq_len, marker_dir=marker_dir)
...
def load_dataset_chb(
    ...
    marker_dir_path = Path(marker_dir) if marker_dir is not None else FILEMARKER_DIR
