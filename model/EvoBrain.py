...
class EvoBrain_classification(nn.Module):
    ...
    def forward(self, input_seq, seq_lengths, adj):
        ...
        node_embeddings = self.gru_gcn(input_seq, adj)
        if self.agg == "concat":
            classifier_in = node_embeddings.view(batch_size, -1)  # (batch_size, num_nodes * num_features)
        else:
            classifier_in = node_embeddings
        logits = self.fc(self.relu(self.dropout(classifier_in)))
        ...
        # Return raw dynamic graph so EEG-RAGNet can refine STGNN-produced structures.
        return pool_logits, node_embeddings, adj
