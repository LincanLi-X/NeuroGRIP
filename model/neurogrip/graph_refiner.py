...
class GraphRefiner(nn.Module):
    ...
    def __init__(
        ...
        max_support_log_records: int = 200,
    ):
        """
        ...
        if match_mode not in {"strict", "soft", "evidence"}:
            raise ValueError("match_mode must be one of: strict, soft, evidence")
        self.match_mode = match_mode
        self.log_supports = log_supports
        self.max_support_log_records = max_support_log_records
    ...
    def _match_indicator(self, entry: dict, node_i_candidates: list, node_j_candidates: list) -> float:
        ...
        Current datasets represent graph endpoints as EEG channels, not explicit
        biomedical entities. When no curated channel-entity map is available,
        strict matching can prune every edge. `match_mode="evidence"` therefore
        uses valid retrieved evidence as the match indicator, while `strict`
        keeps the manuscript's hard entity-match semantics for future curated
        mappings. `soft` tries strict matching first and then falls back to
        evidence only for sufficiently similar retrieved triplets.
        """
        head = str(entry.get("head", "")).strip()
        tail = str(entry.get("tail", "")).strip()
        relation = str(entry.get("relation", "")).strip()
        valid_triplet = bool(head) and bool(tail) and bool(relation)
        if not valid_triplet:
            return 0.0

        if self.match_mode == "evidence":
            return 1.0

        if not node_i_candidates or not node_j_candidates:
            return 0.0 if self.match_mode == "strict" else 1.0
        ...
        return 1.0 if sim01 >= self.match_sim_threshold else 0.0
    ...
    def _save_triplet_supports(self, retrieved_knowledge: list):
        ...
        if len(self._support_records) > self.max_support_log_records:
            self._support_records = self._support_records[-self.max_support_log_records:]
