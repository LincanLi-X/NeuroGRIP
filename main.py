...
def _build_node_embeddings_for_ragnet(hidden, x, num_nodes):
    """
    Build STGNN node embeddings c_t^i for SemAlignQuery.
    Priority:
    1) Model hidden embeddings (expected shape BxNxD)
    2) Temporal hidden embeddings (BxTxNxD -> use last t)
    3) Fallback to input signal temporal average (BxTxNxD -> BxNxD)
    """
...
def _init_ragnet_hook(args, device, input_dim):
...
def main(args):
    ...
    if args.model_name == "dcrnn":
        ...
        raise ValueError(
            ...
            "Use a backbone model (e.g. evobrain/evolvegcn) with --use_ragnet."
        )
    else:
        raise NotImplementedError
                ...
                if args.use_ragnet and (step % args.refine_interval == 0):
                    ...
                    if graph_candidate is None and not warned_graph_fallback:
                        log.warning(
                            ...
                            "EEG-RAGNet currently falls back to dataloader dynamic graph."
                        )
                        warned_graph_fallback = True

                    if raw_graph.dim() == 4:
                        raw_graph_for_refine = raw_graph[:, -1, :, :]
                    elif raw_graph.dim() == 3:
                        raw_graph_for_refine = raw_graph
                    else:
                        raise ValueError(
                            f"Unsupported graph shape for EEG-RAGNet: {tuple(raw_graph.shape)}"
                        )

                    ragnet_in = _build_node_embeddings_for_ragnet(hidden, x, args.num_nodes)
                    if ragnet_hook is None:
                        ragnet_input_dim = ragnet_in.size(-1)
                        if args.semantic_input_dim != ragnet_input_dim:
                            log.warning(
                                "semantic_input_dim (%d) != backbone node embedding dim (%d). "
                                "Using runtime embedding dim for EEG-RAGNet init.",
                                args.semantic_input_dim, ragnet_input_dim
                            )
                        ragnet_hook = _init_ragnet_hook(args, device, input_dim=ragnet_input_dim)
                    if (
                        ...
                        log.warning(
                            ...
                            "EEG-RAGNet falls back to temporal average of input features."
                        )
                        warned_embedding_fallback = True
                    with torch.no_grad():
                        refined_adj = ragnet_hook.refine_graph(raw_graph_for_refine, ragnet_in)

                    if raw_graph.dim() == 4:
                        refined_adj = refined_adj.unsqueeze(1).repeat(1, raw_graph.size(1), 1, 1)

                    # Re-run backbone with refined graph for graph-based backbones.
                    if args.model_name in ("evobrain", "evolvegcn"):
...
def evaluate(
    ...
    with torch.no_grad(), tqdm(total=len(dataloader.dataset)) as progress_bar:
        for x, y, seq_lengths, supports, adj, file_name in dataloader:
            ...
            if args.use_ragnet and (eval_step % args.refine_interval == 0):
                graph_candidate = _extract_graph_candidate(outputs, args.num_nodes)
                raw_graph = graph_candidate if graph_candidate is not None else adj
                if graph_candidate is None and not warned_graph_fallback:
                    print(
                        "[EEG-RAGNet][Eval] Backbone did not return adjacency tensor; "
                ...
                if raw_graph.dim() == 4:
                    ...
                    raise ValueError(f"Unsupported graph shape for EEG-RAGNet eval: {tuple(raw_graph.shape)}")
                ...
                if ragnet_hook is None:
                    ...
                    if args.semantic_input_dim != ragnet_input_dim:
                        print(
                            f"[EEG-RAGNet][Eval] semantic_input_dim ({args.semantic_input_dim}) "
                            f"!= backbone embedding dim ({ragnet_input_dim}); using runtime dim."
                        )
                    ragnet_hook = _init_ragnet_hook(args, device, input_dim=ragnet_input_dim)
                ...
                if (
                    ...
                    print(
                        "[EEG-RAGNet][Eval] Hidden is not node-wise embeddings; "
                        "fallback to temporal average of input features."
                    )
                    warned_embedding_fallback = True
